...
static const uint16_t GATTS_CHAR_UUID_FLAG_GITHUB               = 0xFF17;
...
static const uint8_t client_config_value[2]         = {0x00, 0x00};
//static const uint8_t char_value[4]                 = {0x11, 0x22, 0x33, 0x44};

// start ctf data vars
static char writeData[100];
static char flag_state[21] = "FFFFFFFFFFFFFFFFFFFF";
...
static const char indicate_read_value[] = "Listen to handle 0x0045 for a single indication";
static const char notification_multi_read_value[] = "Listen to me for multi notifications";
static const char indicate_multi_read_value[] = "Listen to handle 0x004d for multi indications";
...
static const char github_value[] = "md5 of author's github handle";
...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_client_config_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      sizeof(client_config_value), sizeof(client_config_value), (uint8_t *)client_config_value}},
    ...
    /* FLAG GitHub Characteristic Declaration */
    [IDX_CHAR_FLAG_GITHUB]      =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
      CHAR_DECLARATION_SIZE, CHAR_DECLARATION_SIZE, (uint8_t *)&char_prop_read}},

    /* FLAG GitHub Characteristic Value */
    [IDX_CHAR_VAL_FLAG_GITHUB]  =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_FLAG_GITHUB, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(github_value)-1, (uint8_t *)github_value}},
...
static uint16_t decode_le16(const uint8_t *value, uint16_t len)
{
    uint16_t decoded = 0;
    if (len > 0) {
        decoded = value[0];
    ...
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
            ...
            if (param->read.handle == blectf_handle_table[IDX_CHAR_FLAG_WRITE_RESPONSE]+1){
                ...
                if (flag_state[16] == 'T' || flag_state[16] == 'H'){
                    static const char write_response_flag[] = "d41d8cd98f00b204e980";
                    rsp.attr_value.len = sizeof(write_response_flag) - 1;
                    memcpy(rsp.attr_value.value, (uint8_t *)write_response_flag, rsp.attr_value.len);
                }else{
                    rsp.attr_value.len = sizeof(write_response_data);
                    memcpy(rsp.attr_value.value, (uint8_t *)write_response_data, rsp.attr_value.len);
                }
        ...
        case ESP_GATTS_WRITE_EVT:
            ...
            if (!param->write.is_prep){
                ...
                size_t write_len = param->write.len;
                if (write_len >= sizeof(writeData)) {
                    write_len = sizeof(writeData) - 1;
                }
                memcpy(writeData, param->write.value, write_len);
                ...
                {
                    uint16_t descr_value = decode_le16(param->write.value, param->write.len);
                ...
                if (blectf_handle_table[IDX_CHAR_FLAG]+1 == param->write.handle)
                {
                    ...
                    if (strcmp(writeData,"637260d8e70610e0d300") == 0){
                        //md5 of device name
                        flag_state[3] = 'T';
                    }
                    ...
                    if (strcmp(writeData,"471b228968e09e9a328f") == 0){
                        //final flag
                        flag_state[19] = 'T';
                    }
        ...
        case ESP_GATTS_CONF_EVT:
            ...
            if (indicate_handle_state == blectf_handle_table[IDX_CHAR_FLAG_NOTIFICATION_MULTI]){
                ...
                vTaskDelay(100);
                esp_ble_gatts_send_indicate(gatts_if, param->conf.conn_id, blectf_handle_table[IDX_CHAR_VAL_FLAG_NOTIFICATION_MULTI], sizeof(indicate_data), (uint8_t *)indicate_data, false);
            }
            // indicate multi
            if (indicate_handle_state == blectf_handle_table[IDX_CHAR_FLAG_INDICATE_MULTI]){
                char indicate_data[20] = "b6f3a47f207d38e16ffa";
                indicate_handle_state = 0;
                esp_ble_gatts_send_indicate(gatts_if, param->conf.conn_id, blectf_handle_table[IDX_CHAR_VAL_FLAG_INDICATE_MULTI], sizeof(indicate_data), (uint8_t *)indicate_data, true);
            }
            break;
